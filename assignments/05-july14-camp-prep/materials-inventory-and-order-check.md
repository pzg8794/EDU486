...
```mermaid
flowchart LR
    E["Evidence says present"] --> C["Count, inspect, clean, and test"]
    C -->|Passes| R["Reserve for Invisible Invaders"]
    C -->|Missing or unusable| G["Record shortage by item ID"]
    G --> P["Select only the needed product"]
    P --> B["Check the $100 budget and delivery time"]
```
